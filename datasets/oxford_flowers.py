...
class OxfordFlowers(DatasetBase):
    ...
    def __init__(self, root, num_shots, data_section):
        ...
        train, val, test, open_set = self.read_split(self.split_path, self.image_dir,
                                           data_section=data_section)  # 分别读取train val 和test的数据集
        train = self.generate_fewshot_dataset(train, num_shots=num_shots)

        super().__init__(train_x=train, val=val, test=test, open_set=open_set)
