...
def run_tip_adapter(cfg, cache_keys, cache_values, val_features, val_labels, test_features, test_labels, open_features, open_labels, clip_weights):
    
    print("\n-------- Searching hyperparameters on the val set. --------")

    # Zero-shot CLIP
    clip_logits = 100. * val_features @ clip_weights  # @是矩阵乘法的操作符
...
def main():
    ...
    dataset = build_dataset(cfg['dataset'], cfg['root_path'], cfg['shots'], cfg['data_section'])  # 建立数据集的同时，初始化了train集

    val_loader = build_data_loader(data_source=dataset.val, batch_size=64, is_train=False, tfm=preprocess, shuffle=False)
    test_loader = build_data_loader(data_source=dataset.test, batch_size=64, is_train=False, tfm=preprocess, shuffle=False)
    open_loader = build_data_loader(data_source=dataset.open_set, batch_size=64, is_train=False, tfm=preprocess, shuffle=False)

    train_tranform = transforms.Compose([
        transforms.RandomResizedCrop(size=224, scale=(0.5, 1), interpolation=transforms.InterpolationMode.BICUBIC),
        transforms.RandomHorizontalFlip(p=0.5),  # 50%的概率将图片翻转
        transforms.ToTensor(),
    ...
    # Pre-load open features
    print("\nLoading visual features and labels from open set.")
    open_features, open_labels = pre_load_features(cfg, "open", clip_model, open_loader)
